...
import com.opentach.common.dms.DmsRuntimeException;
...
public class DocumentationTable extends UTable implements InteractionManagerModeListener {
	...
	private final DocumentationTree								categoryTree;
	private Serializable										currentIdDocument;
	private Map<? extends Serializable, ? extends Serializable>	currentFilter;
	// nhapa
	private boolean												deleting					= false;
	private boolean												ignoreEvents				= false;
	private boolean												ignoreCheckRefreshThread	= false;
	private boolean												categoryPanel				= true;
	private String												form_id_dms_doc_field;
	...
	public DocumentationTable(Hashtable<String, Object> params) throws Exception {
		...
		this.categoryTree = new DocumentationTree(params);
	...
	protected EntityResult doQueryDocuments(Hashtable<?, ?> filter, Vector<?> attrs) throws Exception {
		if (!filter.containsKey(DMSNaming.DOCUMENT_ID_DMS_DOCUMENT)) {
			return new EntityResult();
		}
		return this.getDMSService().fileQuery(filter, attrs, this.getSessionId());
	}

	protected void requeryDocuments() throws Exception {
		...
		final EntityResult er = this.getDMSService().fileQuery(kv, this.getAttributeList(), this.getSessionId());
	...
	public EntityResult updateTable(Hashtable keysValues, int viewColumnIndex, TableCellEditor tableCellEditor, Hashtable otherData, Object previousData) throws Exception {
		...
		service.fileUpdate(fileId, av, this.getSessionId());
		return new EntityResult();
	}
