...
//si no es el móvil
public class LocaleImageTelephoneRow extends LocaleImageRow {
	...
	public LocaleImageTelephoneRow(Hashtable params) {
		super(params);
	}
			...
			try {
				if (this.locator != null) {
					...
					if (IUserData.NIVEL_DOWNLOADTPD4.equals(uData.getLevel())) {
						ipath = this.imagepath + l + "/" + LocaleImageTelephoneRow.IMAGERNAME902;
					} else {
						ipath = this.imagepath + l + "/" + LocaleImageTelephoneRow.IMAGERNAMEMOV;
					}
				...
				LocaleImageTelephoneRow.logger.log(Level.SEVERE, e.getMessage(), e);
			}
		}
	}
}