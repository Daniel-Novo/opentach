 ...
 */
//@formatter:on
public class InsertToExtractionMinusAvailableLaborAgreementAlgorithm implements ILaborAgreementAlgorithm {

	public InsertToExtractionMinusAvailableLaborAgreementAlgorithm() {
		super();
	}
	...
	private int computeWorkingTimeBetween(Date from, Date to, List<Stretch> stretchs) {
		...
		for (Stretch stretch : stretchs) {
			if (!ObjectTools.isIn(stretch.getType(), StretchType.AVAILABLE) && (from.compareTo(stretch.getEndDate()) < 0) && (to.compareTo(stretch.getBeginDate()) > 0)) {
