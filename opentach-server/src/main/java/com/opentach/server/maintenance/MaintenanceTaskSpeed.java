...
public class MaintenanceTaskSpeed extends AbstractMaintenanceTask {
	private static final Logger	logger	= LoggerFactory.getLogger(MaintenanceTaskSpeed.class);
	private int					totalPairs;
	private final AtomicInteger	count	= new AtomicInteger(0);

	@Override
	protected MaintenanceStatusType getTaskKey() {
		return MaintenanceStatusType.DELETE_SPEED;
	}

	@Override
	protected void doInnerMaintenance(String backupFolder, final Timestamp filterDate) throws Exception {
		this.totalPairs = 0;
		final ExecutorService executor = PoolExecutors.newFixedThreadPool("MaintenanceTaskSpeed", 5);
			...
			protected Void doTask(Connection con) throws UException, SQLException {
				return new QueryJdbcTemplate<Void>() {
					...
					protected Void parseResponse(ResultSet rs) throws UException {
						try {
							while (rs.next()) {
								MaintenanceTaskSpeed.this.totalPairs++;
								executor.submit(new DeleteTask(MaintenanceTaskSpeed.this.count, "delete from CDVELOCIDAD where NUMREQ=? AND MATRICULA=? AND FECHORA < ?",
