CREATE OR REPLACE VIEW CDVINFORM_DESCARGA_REMOTA AS 
  SELECT 
CDVEHICULOS_EMP.CIF,
...
CREATE OR REPLACE VIEW CDVINFORM_PENDDESCARGA_AGENTES AS
SELECT DISTINCT
		CG_CONTRATO,
		CIF,
		EMP,      
	...
	FROM
		...
		SELECT 
			...
			0 AS TIPO,
			DFEMP.NOMB               AS EMP,            
			(SELECT MAX(CDFICHEROS.FECFIN) FROM CDFICHEROS,CDFICHEROS_CONTRATO 
			WHERE 
			...
			(SELECT MAX(CDFICHEROS.F_DESCARGA_DATOS) FROM CDFICHEROS,CDFICHEROS_CONTRATO 
			WHERE 
				CDFICHEROS.IDFICHERO   = CDFICHEROS_CONTRATO.IDFICHERO
				AND CDVEHICULO_CONT.CG_CONTRATO = CDFICHEROS_CONTRATO.CG_CONTRATO
				AND CDVEHICULO_CONT.MATRICULA = CDFICHEROS.IDORIGEN
	...
	WHERE 
		((case when MAX_FFIN is null then max_f_descarga_datos when max_f_descarga_datos is null then MAX_FFIN else GREATEST (MAX_FFIN,MAX_F_DESCARGA_DATOS) end) < ( SYSDATE - (CASE WHEN (TIPO=1)	THEN 60	ELSE 120  END))
