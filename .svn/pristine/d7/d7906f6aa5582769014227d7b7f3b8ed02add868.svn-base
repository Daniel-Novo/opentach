...
public class DocumentationTreeModel implements TreeModel {
	...
	private final boolean		showUncategorized;
	 ...
	 * @param params
	 */
	public DocumentationTreeModel(Hashtable<String, Object> params) {
		super();
		this.showUncategorized = ParseUtilsExtended.getBoolean((String) params.get("showuncategorized"), true);
	...
	public void setCategoryTree(DMSCategory category, Serializable idDocument) {
		...
		if (this.showUncategorized) {
			...
			this.rootCategories = category.getChildren();
		}
	...
	public Object getChild(Object parent, int index) {
		if (this.treeRoot.equals(parent)) {
			...
			if (!this.showUncategorized) {
				if ((index >= 0) && ((index) < (this.rootCategories.size()))) {
		...
		} else if (this.showUncategorized && this.uncategorized.equals(parent)) {
			return null;
	...
	public int getChildCount(Object parent) {
		if (this.treeRoot.equals(parent)) {
			...
			return this.showUncategorized ? (1 + this.rootCategories.size()) : this.rootCategories.size();
		} else if (this.showUncategorized && this.uncategorized.equals(parent)) {
			return 0;
		} else if (parent instanceof DMSCategory) {
