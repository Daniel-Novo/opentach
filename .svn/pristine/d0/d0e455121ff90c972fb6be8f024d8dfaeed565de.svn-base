 ...
 * The Class IMUsuario.
 */
public class IMUsuario extends UBasicFIM {
	...
	public void registerInteractionManager(Form f, IFormManager gf) {
		...
		TextDataField cusuario = (TextDataField) this.managedForm.getDataFieldReference("USUARIO");
		cusuario.addValueChangeListener(new ValueChangeListener() {
			@Override
			public void valueChanged(ValueEvent e) {
				if (e.getNewValue() != null) {
					IMUsuario.this.tbUsuDfEmp.refresh();
				}
			}
		});
	}
	...
	private boolean isMonit() {
		try {
			if ((this.monit != null) && "S".equals(this.monit.getValue())) {
