...
public class MaintenanceTaskWorkingPeriods extends AbstractMaintenanceTask {
	private static final Logger	logger	= LoggerFactory.getLogger(MaintenanceTaskWorkingPeriods.class);
	private int					totalPairs;
	private final AtomicInteger	count	= new AtomicInteger(0);

	@Override
	protected MaintenanceStatusType getTaskKey() {
		return MaintenanceStatusType.DELETE_WORKING_PERIODS;
	}

	@Override
	protected void doInnerMaintenance(String backupFolder, final Timestamp filterDate) throws Exception {
		this.totalPairs = 0;
		final ExecutorService executor = PoolExecutors.newFixedThreadPool("MaintenanceTaskWorkingPeriods", 5);
			...
			protected Void doTask(Connection con) throws UException, SQLException {
				return new QueryJdbcTemplate<Void>() {
					...
					protected Void parseResponse(ResultSet rs) throws UException {
						try {
							while (rs.next()) {
								MaintenanceTaskWorkingPeriods.this.totalPairs++;
								executor.submit(
										new DeleteTask(MaintenanceTaskWorkingPeriods.this.count, "delete from CDPERIODOS_TRABAJO where NUMREQ=? AND IDCONDUCTOR=? AND FECINI < ?",
