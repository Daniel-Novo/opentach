...
public class IMInformeUsoVehiculoSinTarjeta extends IMReportRoot {

	public IMInformeUsoVehiculoSinTarjeta() {
		super("EInformeUsoVehiculoSinTarjeta", "informe_uso_vehiculo_sin_tarjeta");
		HashMap<String, String> mEntityReport = new HashMap<String, String>();
		this.setDateTags(new TimeStampDateTags("F_DESCARGA_DATOS"));
		mEntityReport.put("informe_uso_vehiculo_sin_tarjeta", "EInformeUsoVehiculoSinTarjeta");
	...
	public void registerInteractionManager(Form f, IFormManager gf) {
		...
		Table tbUsoVehiculoSinTarjeta = (Table) this.managedForm.getDataFieldReference("EInformeUsoVehiculoSinTarjeta");
		if (tbUsoVehiculoSinTarjeta != null) {
			MinutesCellRender mincr = new MinutesCellRender("MINUTOS");
			mincr.setSecondsResolution(true);
			tbUsoVehiculoSinTarjeta.setRendererForColumn("MINUTOS", mincr);
		}
		...
		bt.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent actionevent) {
				IMInformeUsoVehiculoSinTarjeta.this.createReports();
			}
		});
	...
	private void createReports() {
		...
		final JRReportDescriptor jrd = jpm.getDataMap().get(55);
			...
			try {
				...
				OperationThread opth = new OperationThread() {
					...
					public void run() {
						...
						JasperPrint jp = null;
						try {
							Table tb = (Table) IMInformeUsoVehiculoSinTarjeta.this.managedForm.getElementReference("EInformeUsoVehiculoSinTarjeta");
							jp = ocl.getRemoteService(IReportService.class).fillReport(urljr, params, null, null,
									(EntityResult)tb.getValue(), ocl.getSessionId());
						} catch (Exception e) {
							e.printStackTrace();
						} finally {
