...
public class SessionUnconnectedUsersCard extends AbstractStatisticsCard<Row> {

	private static final Logger	logger	= LoggerFactory.getLogger(SessionUnconnectedUsersCard.class);
	private Table				table;

	public SessionUnconnectedUsersCard(OpentachClientLocator locator, Form form, int xSize, int ySize) {
	...
	public void refresh() throws UException {
		try {
			EntityResult er = this.getStatsReportService().getSessionUnconnectedUsers(this.getQueryFilter(), this.getGroupingTime(), this.getSessionId());
