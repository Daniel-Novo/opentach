...
public class MaintenanceTaskVehicleUses extends AbstractMaintenanceTask {
	private static final Logger	logger	= LoggerFactory.getLogger(MaintenanceTaskVehicleUses.class);
	private int					totalPairs;
	private final AtomicInteger	count	= new AtomicInteger(0);

	@Override
	protected MaintenanceStatusType getTaskKey() {
		return MaintenanceStatusType.DELETE_VEHICLE_USES;
	}

	@Override
	protected void doInnerMaintenance(String backupFolder, final Timestamp filterDate) throws Exception {
		this.totalPairs = 0;
		final ExecutorService executor = PoolExecutors.newFixedThreadPool("MaintenanceTaskVehicleUses", 5);
			...
			protected Void doTask(Connection con) throws UException, SQLException {
				return new QueryJdbcTemplate<Void>() {
					...
					protected Void parseResponse(ResultSet rs) throws UException {
						try {
							while (rs.next()) {
								MaintenanceTaskVehicleUses.this.totalPairs++;
								executor.submit(new DeleteTask(MaintenanceTaskVehicleUses.this.count,
										"delete from CDUSO_VEHICULO where NUMREQ=? AND " + rs.getString("SOURCENAME") + "=? AND FECINI < ?", rs.getObject("CG_CONTRATO"),
