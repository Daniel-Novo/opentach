...
public class PurgeCDREGKM_CONDUCTOR {

	private static final Logger	logger		= LoggerFactory.getLogger(PurgeCDREGKM_CONDUCTOR.class);
	private static final String	NUM_DAYS	= "900";

	public PurgeCDREGKM_CONDUCTOR() {
		...
		try {
			...
			if (res != null) {
				...
				for (int i = 0; i < nregs; i++) {
					...
					PurgeCDREGKM_CONDUCTOR.queueIds.add(new Pair(listNumreq.get(i), listIdconductor.get(i)));
				}
			}

			List<Thread> purgeThreads = new ArrayList<Thread>();
			for (int endNumber = 0; endNumber < 15; endNumber++) {
				Thread th = new Thread(new PurgeCdRegKmConductorTask(sqlEntity, sessionId, startTime, count), "purge-" + endNumber);
				purgeThreads.add(th);
				th.start();
			}
	...
	private static class PurgeCdRegKmConductorTask implements Runnable {
		...
		public PurgeCdRegKmConductorTask(IMultiConnectionSqlExecutor sqlEntity, int sessionId, long startTime, AtomicInteger count) {
		...
		public void run() {
			while (true) {
				try {
					Pair<Object, Object> info = PurgeCDREGKM_CONDUCTOR.queueIds.poll();
					if (info == null) {
					...
					EntityResult response = this.sqlEntity
							.execute(
									null,
									"delete from TACHBB102011.CDREGKM_CONDUCTOR where NUMREQ='" + numreq + "' AND IDCONDUCTOR='" + idconductor + "' AND FECHA < (sysdate - " + PurgeCDREGKM_CONDUCTOR.NUM_DAYS + ")",
									this.sessionId);
