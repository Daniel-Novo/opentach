...
public class IdentifiedFocusTraversalPolicy extends LayoutFocusTraversalPolicy {
	...
	static class PriorComparator implements Comparator, java.io.Serializable {
		...
		public int compare(Object o1, Object o2) {
			...
			Container aFormComponent = getFormComponentAncestorOfClass(FormComponent.class, a);
			Object aAttr = null;
			if (aFormComponent instanceof IdentifiedElement) {
				aAttr = ((IdentifiedElement) aFormComponent).getAttribute();
			}
			Container bFormComponent = getFormComponentAncestorOfClass(FormComponent.class, b);
			Object bAttr = null;
			...
			/*
			 * Compare Function compare(a,b) if a < b => return -1 if a = b =>
			 * return 0 if a > b => return 1
			 */

			if ((aAttr == null) && (bAttr == null)) { // The components are not
															  // IdentifiedElements. They are
															  // ordered by position.
				return comparePosition(a, b);
			} else if ((aAttr != null) && (bAttr == null)) { // Just component "a" is
																		// IdentifiedElement. The
																		// element "a" have to be
																		// placed before.
				return -1;
			} else if ((aAttr == null) && (bAttr != null)) { // Just component "b" is
																		// IdentifiedElement. The
																		// element "b" have to be
																		// placed before.
				return 1;
				...
				if ((indA != -1) && (indB != -1)) { // If both components are included
														  // into orderVector...
					if (indA < indB) {
						return -1;
					} else if (indA > indB) {
						...
						// The Focus is given by position to the first left element.
				...
				} else if ((indA != -1) && (indB == -1)) { // Just the element "a"
																	// belongs to the orderVector
					return -1;
				} else if ((indA == -1) && (indB != -1)) { // Just the element "b"
																	// belongs to the orderVector
					return 1;
				} else {
					return comparePosition(a, b);
		...
		public static int getX(Container commonParent, Component c) {
			if (commonParent == null) return c.getX();
			else {
				int x = c.getX();
