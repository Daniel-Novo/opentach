...
public class IMInformeVehAgente extends IMReportRoot {

	public IMInformeVehAgente() {
		super("EInformeVehiculosAgente", "informe_vehiculosAgente");
		HashMap<String, String> mEntityReport = new HashMap<String, String>();
		mEntityReport.put("informe_vehiculosAgente", "EInformeVehiculosAgente");
	...
	public void registerInteractionManager(Form f, IFormManager gf) {
		...
		bt.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent actionevent) {
				...
				IMInformeVehAgente.this.createReports();
			}
		});
	...
	protected void createReports() {
		...
		final Map<String, Object> params = this.getParams("informe_vehiculosAgente", "iddelegacion");
		...
		try {
			final String urljr = "com/opentach/reports/vehAgente.jasper";
			OperationThread opth = new OperationThread() {
				...
				public void run() {
					...
					JasperPrint jp = null;
					try {
						EntityResult res = (EntityResult)((Table)managedForm.getDataFieldReference("EInformeVehiculosAgente")).getValue();
			...
			if ((jp != null) && (jp.getPages() != null) && (jp.getPages().size() > 0)) {
				JRDialogViewer jv = JRDialogViewer.getJasperViewer(ApplicationManager.getTranslation("informe_vehiculosAgente"), jd, jp);
				jv.setVisible(true);
			} else {
