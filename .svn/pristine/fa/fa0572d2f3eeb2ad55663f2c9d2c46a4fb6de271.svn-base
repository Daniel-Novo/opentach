...
public class CustomClassicLayoutManager extends ClassicLayoutManager {
	...
	protected void layoutGroupFooterLabels(DJGroup djgroup, JRDesignGroup jgroup) {
		...
		if (this.isShowedRowNumber) {
			labelTf.setX((width / Integer.parseInt(this.hColsPositions.get(col.getName()).toString())) + DynamicJasperEngine.widthRowNumbers);
		} else {
	...
	protected void transformDetailBand() {
		...
		for (Iterator iter = this.getVisibleColumns().iterator(); iter.hasNext();) {
			...
			if (column instanceof BarCodeColumn) {
				...
				imageExp.setText("ar.com.fdvs.dj.core.BarcodeHelper.getBarcodeImage(" + barcodeColumn.getBarcodeType() + ", " + column.getTextForExpression() + ", " + barcodeColumn
						.isShowText() + ", " + barcodeColumn.isCheckSum() + ", " + applicationIdentifier + ",0,0 )");
	...
	private void setUpConditionStyles(JRDesignStyle jrstyle, AbstractColumn column) {
		...
		for (Iterator iterator = column.getConditionalStyles().iterator(); iterator.hasNext();) {
			...
			if (this.getReport().getOptions().isPrintBackgroundOnOddRows() && (Transparency.TRANSPARENT == condition.getStyle().getTransparency())) { // condition
				// style
				// +
				// odd
				// row
				...
				expressionOdd.setText(
						"new java.lang.Boolean(" + AbstractLayoutManager.EXPRESSION_TRUE_WHEN_ODD + ".booleanValue() && ((java.lang.Boolean)" + expStr + ").booleanValue() )");
				...
				expressionEven.setText(
						"new java.lang.Boolean(" + AbstractLayoutManager.EXPRESSION_TRUE_WHEN_EVEN + ".booleanValue() && ((java.lang.Boolean)" + expStr + ").booleanValue() )");
	...
	protected void layoutGroupVariables(DJGroup group, JRDesignGroup jgroup, int labelOffset) {
		...
		Integer height = group.getHeaderVariablesHeight() != DynamicReportOptions.UNSET_VALUE ? group.getHeaderVariablesHeight() : this.getReport().getOptions().getDetailHeight();
		...
		// VALUE_IN_HEADER_WITH_HEADERS_AND_COLUMN_NAME
			...
			if (((DynamicJasperEngine) this.reportEngine).isColumnGroupInSimpleGroup(group.getName().substring(18))) {
				...
				for (int i = vMultiGroupColumns.size() - 1; i >= 0; i--) {
					JRDesignTextField columnMultiNameTf = this.createColumnNameTextField(group, this.getColumnFromVirtualColumn((AbstractColumn) vMultiGroupColumns.get(i)),
					...
					// textfield for the current value
					JRDesignTextField currentMultiValue = this.generateTextFieldFromColumn((AbstractColumn) vMultiGroupColumns.get(i), height.intValue(), group,
							i == (vMultiGroupColumns.size() - 1) ? false : true);
					...
					// width)
					currentMultiValue.setWidth(this.getReport().getOptions().getPrintableWidth() - columnNameTf.getWidth());
		...
		this.placeVariableInBand(group.getHeaderVariables(), group, jgroup, DJConstants.HEADER, headerBand, headerOffset);
		this.placeVariableInBand(group.getFooterVariables(), group, jgroup, DJConstants.FOOTER, footerBand, labelOffset);
	}
	...
	protected void layoutGroups() {
		for (Iterator iter = this.getReport().getColumnsGroups().iterator(); iter.hasNext();) {
			...
			if (columnsGroup.getLayout().isPrintHeaders()) {
				...
				if (groupIdx > 0) {
					DJGroup prevG = this.getReport().getColumnsGroups().get(groupIdx - 1);
				...
				for (Iterator iterator = this.getVisibleColumns().iterator(); iterator.hasNext();) {
					...
					if (col.getTitle().length() > 12) {
						...
						if ((width.intValue() * 1.5) < FontHelper.getWidthFor(col.getStyle().getFont(), col.getTitle())) {
							col.setTitle(this.getFittingTextForWidth((int) (width.intValue() * 1.5), col.getTitle(), col.getStyle().getFont()));
						}
					}
	...
	protected JRDesignTextField createColumnNameTextField(DJGroup columnsGroup, AbstractColumn col, boolean requireMultiGroupStyle) {
		JRDesignTextField designStaticText = new JRDesignTextField();
	...
	protected void placeVariableInBand(List variables, DJGroup djGroup, JRDesignGroup jgroup, String type, JRDesignBand band, int yOffset) {
		...
		while (it.hasNext()) {
			...
			if (this.isShowedRowNumber) {
				textField.setX(
						(((this.getReport().getOptions().getPrintableWidth() - DynamicJasperEngine.widthRowNumbers) / (this.hColsPositions.size() == 0 ? 1 : this.hColsPositions
								.size())) * posColumn) + DynamicJasperEngine.widthRowNumbers);
			} else {
				textField.setX((this.getReport().getOptions().getPrintableWidth() / (this.hColsPositions.size() == 0 ? 1 : this.hColsPositions.size())) * posColumn);
			}
			...
			Style defStyle = DJConstants.HEADER.equals(type) ? djGroup.getDefaulHeaderVariableStyle() : djGroup.getDefaulFooterVariableStyle();
	...
	protected JRDesignTextField generateTextFieldFromColumn(AbstractColumn col, int height, DJGroup group, boolean requireMultiGroupStyle) {
		...
		if (group != null) {
			...
			/**
			 * Since a group column can share the style with non group columns, if oddRow coloring is enabled, we modified this shared style to have a colored background on odd
			 * rows. We don't want that for group columns, that's why we create our own style from the existing one, and remove proper odd-row conditional style if present
			 */
	...
	public AbstractColumn getColumnFromVirtualColumn(AbstractColumn virtualColumn) {
		if (((DynamicJasperEngine) this.reportEngine).isVirtualColumn(virtualColumn.getName())) {
			for (int i = 0; i < this.getVisibleColumns().size(); i++) {
				if (this.getVisibleColumns().get(i).getName()
						.equals(((DynamicJasperEngine) this.reportEngine).getColumnFromVirtualColumn(virtualColumn.getName()))) {
					return this.getVisibleColumns().get(i);
				}
			}
		}
