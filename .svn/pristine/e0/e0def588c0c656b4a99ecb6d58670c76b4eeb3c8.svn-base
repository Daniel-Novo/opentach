...
public class EInformekmsEmpresa extends FileTableEntity {

	// private static final Logger logger = LoggerFactory.getLogger(EInformekms.class);

	public EInformekmsEmpresa(EntityReferenceLocator b, DatabaseConnectionManager g, int p) throws Exception {
		super(b, g, p);
	}
	...
	public EntityResult query(Hashtable cv, Vector v, int sesionId, Connection con) throws Exception {
		...
		try {
			...
			if ((resDrivers != null) && (resDrivers.calculateRecordNumber() > 0)) {
				...
				String sqlconductores = new Template("sql/EInformekmsRecorridosEmpresa.sql")
