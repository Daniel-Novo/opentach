...
public class IMInformeKMRecorridosUsosCond extends IMReportRoot {

	public IMInformeKMRecorridosUsosCond() {
		super("EInformeUsoTarjeta", "informe_km_cond");
		this.dateEntity = "EUFecha";
		HashMap<String, String> mEntityReport = new HashMap<String, String>();
		mEntityReport.put("informe_km_cond", "EInformeUsoTarjeta");
		this.setEntidadesInformes(mEntityReport);
		this.setDateTags(new TimeStampDateTags("FECINI"));
	}
	...
	public void registerInteractionManager(Form f, IFormManager gf) {
		...
		bt.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent actionevent) {
				IMInformeKMRecorridosUsosCond.this.createReports();
			}
		});
	}
	...
	protected void createReports() {
		...
		final Map<String, Object> params = this.getParams("managerkmrecorridosconductorusos", "iddelegacion");
		...
		try {
			final String urljr = "com/opentach/reports/kmrecorridosusoscond.jasper";
			JRPropertyManager jpm = new JRPropertyManager("com/opentach/reports/reports.properties");
			final JRReportDescriptor jrd = jpm.getDataMap().get(42);
			OperationThread opth = new OperationThread() {
				...
				public void run() {
					...
					JasperPrint jp = null;
					try {
						Table tb = (Table) IMInformeKMRecorridosUsosCond.this.managedForm.getElementReference("EInformeUsoVehiculoConductor");
			...
			if ((jp != null) && (jp.getPages() != null) && (jp.getPages().size() > 0)) {
				JRDialogViewer jv = JRDialogViewer.getJasperViewer(ApplicationManager.getTranslation("informe_km_cond"), jd, jp);
				jv.setVisible(true);
			} else {
