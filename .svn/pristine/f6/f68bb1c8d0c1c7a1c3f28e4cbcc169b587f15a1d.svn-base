...
public class IMInformeConexionesUsu extends IMReportRoot {

	private static final Logger	logger	= LoggerFactory.getLogger(IMInformeConexionesUsu.class);

	public IMInformeConexionesUsu() {
		super("EInformeConexionesUsu", "informe_conexiones_usu");
		HashMap<String, String> mEntityReport = new HashMap<String, String>();
		this.setDateTags(new TimeStampDateTags("F_INI"));
		mEntityReport.put("informe_conexiones_usu", "EInformeConexionesUsu");
	...
	public void registerInteractionManager(Form f, IFormManager gf) {
		...
		bt.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent actionevent) {
				IMInformeConexionesUsu.this.createReports();
			}
		});
	...
	public void setInitialState() {
		...
		try {
			...
			MessageManager.getMessageManager().showExceptionMessage(ex, IMInformeConexionesUsu.logger);
		}
	}
	...
	private void createReports() {
		...
		final JRReportDescriptor jrd = jpm.getDataMap().get(48);
			...
			try {
				...
				OperationThread opth = new OperationThread() {
					...
					public void run() {
						...
						JasperPrint jp = null;
						try {
							Table tb = (Table) IMInformeConexionesUsu.this.managedForm.getElementReference("EInformeConexionesUsu");
	...
	protected Map<String, Object> getDateFilterValues(List<String> keys) {
		...
		try {
			...
			IMInformeConexionesUsu.logger.error(null, e);
		}
		return null;
	}
}
