...
public class IMInformekmsEmpresa extends IMReportRoot {

	private static final Logger	logger	= LoggerFactory.getLogger(IMInformekmsEmpresa.class);

	public IMInformekmsEmpresa() {
		super("EInformekmsEmpresa", "informe_kms_empresa");
		HashMap<String, String> mEntityReport = new HashMap<String, String>();
		this.setDateTags(new TimeStampDateTags("F_ALTA"));
		mEntityReport.put("informe_kms_empresa", "EInformekmsEmpresa");
	...
	public void registerInteractionManager(Form f, IFormManager gf) {
		...
		bt.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent actionevent) {
				IMInformekmsEmpresa.this.createReports();
			}
		});
	...
	public void setInitialState() {
		...
		try {
			...
			MessageManager.getMessageManager().showExceptionMessage(ex, IMInformekmsEmpresa.logger);
		}
	}
			...
			try {
				...
				OperationThread opth = new OperationThread() {
					...
					public void run() {
						...
						JasperPrint jp = null;
						try {
							Table tb = (Table) IMInformekmsEmpresa.this.managedForm.getElementReference("EInformekmsEmpresa");
