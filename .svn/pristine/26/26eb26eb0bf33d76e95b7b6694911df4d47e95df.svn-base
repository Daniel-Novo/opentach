 ...
 */
public class InfractionAnalyzerInServerForReportTask extends AbstractDelegate implements Callable<Void>, Priorizable {

	private static final Logger			logger			= LoggerFactory.getLogger(InfractionAnalyzerInServerForReportTask.class);
	 ...
	 *            the priority
	 */
	public InfractionAnalyzerInServerForReportTask(Connection conn, Scale scale, AnalysisParameters parameters, Object contract, Object cif, Object driver, Date beginDate,
	...
	private void analyze() throws Exception {
		...
		new QueryJdbcTemplate<Void>() {
			...
			protected Void parseResponse(ResultSet rs) throws UException {
				try {
					InfractionAnalyzerInServerForReportTask.this.analyze(new ActivityIterator(rs));
					return null;
				} catch (Exception ex) {
	...
	private void analyze(Iterator<InputActivity> activityIterator) throws Exception {
		...
		InfractionAnalyzerInServerForReportTask.logger.warn("Tiempo analisis {}, saveToDb {}, total {}", generatorTime, saveToDb, chrono.stopMs());
	}
	...
	private void saveRests(Map<RestClass, List<Rest>> rests) throws Exception {
		...
		new BatchUpdateJdbcTemplate<Rest>(restList) {
			...
			protected Object[] beanToParametersArray(int idx, Rest rest) {
				...
				array[0] = InfractionAnalyzerInServerForReportTask.this.driver;
				...
				array[6] = InfractionAnalyzerInServerForReportTask.ID_GENERATOR.getAndIncrement();
	...
	private void savePeriods(final Map<PeriodClass, List<Period>> periods) throws Exception {
		...
		new BatchUpdateJdbcTemplate<Period>(periodList) {
			...
			protected Object[] beanToParametersArray(int idx, Period period) {
				...
				array[0] = InfractionAnalyzerInServerForReportTask.ID_GENERATOR.getAndIncrement();
				array[1] = InfractionAnalyzerInServerForReportTask.this.driver;
	...
	private void saveStretchs(final LinkedList<Stretch> stretchs) throws Exception {
		new BatchUpdateJdbcTemplate<Stretch>(stretchs) {
			...
			protected Object[] beanToParametersArray(int idx, Stretch stretch) {
				...
				array[5] = InfractionAnalyzerInServerForReportTask.this.driver;
