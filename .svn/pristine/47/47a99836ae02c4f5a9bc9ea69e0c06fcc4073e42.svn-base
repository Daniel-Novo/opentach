...
public class IMInformeIncidResumen extends IMInformeResumen {
		...
		if (this.bInforme2 != null) {
			this.bInforme2.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					IMInformeIncidResumen.this.accBotInforme2();
				}
			});
		}
	}
	...
	private void createReports() {
		...
		final JRReportDescriptor jrd = jpm.getDataMap().get(9);
						...
						public void run() {
							...
							try {
								...
								for (int i = 0; i < lReports.size(); i++) {
									...
									if (i != 0) {
										p = IMInformeIncidResumen.this.getParams(jru.getDscr(), jru.getDelegCol());
									}
