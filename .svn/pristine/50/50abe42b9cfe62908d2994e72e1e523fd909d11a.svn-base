...
public class IMInformeEmpresaFlota extends IMReportRoot {

	public IMInformeEmpresaFlota() {
		super("EInformeEmpresaFlota", "informe_empresaFlota");
		HashMap<String, String> mEntityReport = new HashMap<String, String>();
		mEntityReport.put("informe_empresaFlota", "EInformeEmpresaFlota");
	...
	public void registerInteractionManager(Form f, IFormManager gf) {
		...
		bt.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent actionevent) {
				IMInformeEmpresaFlota.this.createReports();
			}
		});
	...
	public void setInitialState() {
		...
		((Table)managedForm.getElementReference("EInformeEmpresaFlota")).refresh();
	}

	protected void createReports() {

		final Map<String, Object> params = this.getParams("informe_empresaFlota", "iddelegacion");
		...
		try {
			final String urljr = "com/opentach/reports/empresaFlota.jasper";
			OperationThread opth = new OperationThread() {
				...
				public void run() {
					...
					JasperPrint jp = null;
					try {
						EntityResult res = (EntityResult)((Table)managedForm.getDataFieldReference("EInformeEmpresaFlota")).getValue();
			...
			if ((jp != null) && (jp.getPages() != null) && (jp.getPages().size() > 0)) {
				JRDialogViewer jv = JRDialogViewer.getJasperViewer(ApplicationManager.getTranslation("informe_empresaFlota"), jd, jp);
				jv.setVisible(true);
			} else {
