...
public class IMInformeCooperativas extends IMReportRoot {

	private static final Logger	logger	= LoggerFactory.getLogger(IMInformeCooperativas.class);

	public IMInformeCooperativas() {
		super("EInformeCooperativas", "informe_cooperativas");
		HashMap<String, String> mEntityReport = new HashMap<String, String>();
		this.setDateTags(new TimeStampDateTags("F_ALTA"));
		mEntityReport.put("informe_cooperativas", "EInformeCooperativas");
	...
	public void registerInteractionManager(Form f, IFormManager gf) {
		...
		bt.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent actionevent) {
				IMInformeCooperativas.this.createReports();
			}
		});
	...
	public void setInitialState() {
		...
		try {
			...
			MessageManager.getMessageManager().showExceptionMessage(ex, IMInformeCooperativas.logger);
		}
	}
	...
	private void createReports() {
		...
		final JRReportDescriptor jrd = jpm.getDataMap().get(47);
			...
			try {
				...
				OperationThread opth = new OperationThread() {
					...
					public void run() {
						...
						JasperPrint jp = null;
						try {
							Table tb = (Table) IMInformeCooperativas.this.managedForm.getElementReference("EInformeCooperativas");
	...
	protected Map<String, Object> getDateFilterValues(List<String> keys) {
		...
		try {
			...
			IMInformeCooperativas.logger.error(null, e);
		}
		return null;
	}
}
