...
public class IMInformeVehAltaBaja extends IMReportRoot {

	public IMInformeVehAltaBaja() {
		super("EVehiculosEmp", "informe_vehAltaBaja");
		HashMap<String, String> mEntityReport = new HashMap<String, String>();
		mEntityReport.put("informe_vehAltaBaja", "EVehiculosEmp");
	...
	public void registerInteractionManager(Form f, IFormManager gf) {
		...
		bt.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent actionevent) {
				...
				IMInformeVehAltaBaja.this.createReports();
			}
		});
	...
	protected void createReports() {
		...
		final Map<String, Object> params = this.getParams("informe_vehAltaBaja", "iddelegacion");
		...
		try {
			final String urljr = "com/opentach/reports/vehAltaBaja.jasper";
			OperationThread opth = new OperationThread() {
				...
				public void run() {
					...
					JasperPrint jp = null;
					try {
						EntityResult res = (EntityResult)((Table)IMInformeVehAltaBaja.this.managedForm.getDataFieldReference("EVehiculosEmp")).getValue();
			...
			if ((jp != null) && (jp.getPages() != null) && (jp.getPages().size() > 0)) {
				JRDialogViewer jv = JRDialogViewer.getJasperViewer(ApplicationManager.getTranslation("informe_VehAltaBaja"), jd, jp);
				jv.setVisible(true);
			} else {
