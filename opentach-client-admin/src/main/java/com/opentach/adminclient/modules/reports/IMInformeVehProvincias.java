...
public class IMInformeVehProvincias extends IMReportRoot {

	public IMInformeVehProvincias() {
		super("EVehiculosEmp", "InformeVehProvincias");
		HashMap<String, String> mEntityReport = new HashMap<String, String>();
		mEntityReport.put("InformeVehProvincias", "EVehiculosEmp");
	...
	public void registerInteractionManager(Form f, IFormManager gf) {
		...
		bt.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent actionevent) {
				IMInformeVehProvincias.this.createReports();
			}
		});
	...
	protected void createReports() {
		...
		final Map<String, Object> params = this.getParams("InformeVehProvincias", "iddelegacion");
		...
		try {
			final String urljr = "com/opentach/reports/vehprovincias.jasper";
			OperationThread opth = new OperationThread() {
				...
				public void run() {
					...
					JasperPrint jp = null;
					try {
						EntityResult res = (EntityResult)((Table)managedForm.getDataFieldReference("EVehiculosEmp")).getValue();
			...
			if ((jp != null) && (jp.getPages() != null) && (jp.getPages().size() > 0)) {
				JRDialogViewer jv = JRDialogViewer.getJasperViewer(ApplicationManager.getTranslation("InformeVehProvincias"), jd, jp);
				jv.setVisible(true);
			} else {
