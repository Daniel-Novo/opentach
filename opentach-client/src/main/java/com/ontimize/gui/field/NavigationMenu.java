...
public class NavigationMenu extends JPanel {
	...
	protected static final String TOOLTIPS = "tooltips";
	/**
	...
	private static final Class[] params = { String.class, String[].class, ImageIcon[].class, int.class, int.class, int.class, int.class, Color.class, Color.class, Color.class,
			Color.class };
	/**
	 * Message error.
	 */
	...
	protected MenuGroup createMenuGroup(Node node) throws Exception {
		...
		String[] tips = null;
		currentNode = this.getNode(attributes, NavigationMenu.TOOLTIPS);
		if (currentNode != null) {
			String tooltips = currentNode.getNodeValue();
			ArrayList li = new ArrayList();
			StringTokenizer tokens = new StringTokenizer(tooltips, ";");
			while (tokens.hasMoreTokens()) {
				li.add(tokens.nextToken());
			}
			tips = (String[]) li.toArray(new String[li.size()]);
		}
		...
		return this.createMenuGroupInstance(header, opts, icons, tips,  x, y, width, height, bgHeader, bgBody, fg, border);
	}
	...
	protected MenuGroup createMenuGroupInstance(String header, String[] opts, ImageIcon[] icons, String[] tips, int x, int y, int width, int height, Color bgHeader, Color bgBody, Color fg,
		...
		Object ob = constructors
				.newInstance(new Object[] { header, opts, icons, tips, new Integer(x), new Integer(y), new Integer(width), new Integer(height), bgHeader, bgBody, fg, border });
		return (MenuGroup) ob;
	}
	...
	public static class MenuGroup extends JComponent {
		 ...
		 *            The color of the MenuGroup font.
		 * @param borderColor
		 *            The color of the border of the MenuGroup. Use {@link #MenuGroup(Hashtable parameters)}
		 * @deprecated
		 */
		@Deprecated
		public MenuGroup(String header, String[] options, ImageIcon[] icons,String[] tooltips, int x, int y, int w, int h, Color bgH, Color bgB, Color fg, Color borderColor) {
			...
			param.put(NavigationMenu.TOOLTIPS, tooltips);
		 ...
		 *
		 * @param parameters
		 *            the <code>Hashtable</code> with the whole parameters to configure the MenuGroup.
		 *            <p>
		 ...
		 *            <td></td>
		 *            <td>no</td>
		 *            <td>The absolute height of the MenuGroup. The header height is included into this height.</td>
		 *            </tr>
		 *            <tr>
		 *            <td>bgbody</td>
		 *            <td>Color: "214;222;230" or "#ffffff"</td>
		 ...
		 *            <td>false</td>
		 *            <td>no</td>
		 *            <td>Specifies if all the MenuGroup could be dragged or only the header. By default, only header is draggable.</td>
		 *            </tr>
		 *            <td>separator</td>
		 *            <td>"yes/no" or "true/false"</td>
		 *            <td>true</td>
		...
		public MenuGroup(Hashtable parameters) {
			...
			this.icon = ParseUtils.getImageIcon((String) ApplicationManager.getParameterValue(NavigationMenu.MENU_GROUP_HEADER_ICON, parameters),
		...
		protected void createMenuItems(Hashtable parameters) {
			...
			try {
				...
				for (int i = 0; i < options.length; i++) {
					Constructor constructor = itemClass.getConstructor(new Class[] { String.class, ImageIcon.class , String.class});
					this.menuItem[i] = (MenuItem) constructor.newInstance(new Object[] { options[i], icons[i], tooltips == null ? null : tooltips[i] });
				}
			} catch (Exception ex) {
	...
	public static class MenuItem implements ActionListener {
		 ...
		 */
		public MenuItem(String manager, ImageIcon icon, String ttip) {
			super();
			this.icon = icon;
		 ...
		 */
		public MenuItem(String manager, ImageIcon icon, String ttip, boolean visible, boolean enabled) {
			this.manager = manager;
			this.icon = icon;
	...
	public static class DefaultMenuItemRenderer extends JLabel implements MenuItemRenderer, Internationalization {
		...
		public Component getMenuItemRendererComponent(MenuGroup group, MenuItem item, boolean selected, boolean enabled) {
			...
			if (item.getToolTip()!=null) {
				...
				//				ApplicationManager.getTranslation(item.getToolTip(), this.bundle)
			}
