...
public class IMNotices extends UBasicFIM {
	...
	public static final String	MY_RECEIVED_ALERT_NOTICES_TABLE	= "EAvisosRecibidosAlert";
	...
	public void setUpdateMode() {
		...
		if (this.formManager.getReferenceLocator() instanceof INoticeSystem) {
			try {
				...
				this.tMyReceivedAlertNotices.refresh(true);
			} catch (Exception e) {
				e.printStackTrace();
			}
		}

		// formularioGestionado.getBoton("Permisos").setEnabled(true);
	}

	// public void setTemplateManager(ITemplateManagement manager) {
	...
	public void registerInteractionManager(Form f, IFormManager gf) {
		...
		this.tMyReceivedUserNotices = (Table) this.managedForm.getElementReference(IMNotices.MY_RECEIVED_USER_NOTICES_TABLE);
		...
		this.tMyReceivedAlertNotices = (Table) this.managedForm.getElementReference(IMNotices.MY_RECEIVED_ALERT_NOTICES_TABLE);
		this.htmlTemplate = (HTMLTemplate) this.managedForm.getElementReference("htmltemplate");
		// Se incluye un escuhador para la tabla de avisos recibidos para que al
		// seleccioanar un aviso se muestre su información en el componente HTML
		if (this.tMyReceivedAlertNotices != null) {
			this.tMyReceivedAlertNotices.getJTable().getSelectionModel()
			.addListSelectionListener(new ReceivedTableListener(this.tMyReceivedAlertNotices));
		}
	...
	class ReceivedTableListener implements ListSelectionListener {
		...
		public void valueChanged(ListSelectionEvent e) {
			...
			} else if (IMNotices.this.htmlTemplate != null) {
				IMNotices.this.htmlTemplate.delete();
