...
public class IMGraficaCondIndef extends IMDataRoot {
	...
	public void registerInteractionManager(Form f, IFormManager gf) {
		...
		if (tbPeriodos != null) {
			tbPeriodos.getJTable().setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
...
//			});
	...
	private void generateTemplate() {
		...
		if (tb.getSelectedRow() < 0) {
			this.managedForm.message("E_SELECT_ONE_RECORD", Form.INFORMATION_MESSAGE);
			return;
		}
	...
	private Hashtable<String, Object>  getParams() throws Exception {
		...
		if (tb != null) {
			...
			Hashtable<String, Object> avSelected = tb.getSelectedRowData();
			dinicio = (Date) ((Vector) avSelected.get("FECINI")).get(0);
			dfin = (Date) ((Vector) avSelected.get("FECFIN")).get(0);
			...
			if (dfin != null) {
				...
				params.put(IMGraficaCondIndef.M21, df.format(dfin));
			} else {
